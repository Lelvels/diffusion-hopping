...
def setup_model_and_data_module(dataset_name, checkpoint_path, device="cpu"):
...
def main():
    parser = argparse.ArgumentParser(
        prog="evaluate_local.py",
        description="Evaluate ligand generation",
        epilog="Example: python evaluate_local.py local_test pdbbind_filtered --checkpoint_path /path/to/ckpt",
    )
    parser.add_argument(
        "run_id",
        type=str,
        help="Run id of the model to evaluate (used for output directory naming)",
    )
    ...
    model, data_module = setup_model_and_data_module(
        dataset_name, checkpoint_path, device=device
    )
