#!/bin/bash -l
#SBATCH --job-name=train_diffhopp
...
#SBATCH --gres=gpu:1
#SBATCH --output="/net/storage/pr3/plgrid/plggsball/plgkietho/maidung/Code/pharma_res/results/diff_hopp/logs/train/train_model_%j.log"
#SBATCH --error="/net/storage/pr3/plgrid/plggsball/plgkietho/maidung/Code/pharma_res/results/diff_hopp/logs/train/train_model_%j.err"
...
echo ""
echo "Checking pytorch_lightning..."
$pybin -c "import pytorch_lightning; print('✓ pytorch_lightning import successful')" 2>&1
if [[ $? -ne 0 ]]; then
    echo "ERROR: Failed to import pytorch_lightning" >&2
    exit 1
fi

$pybin -c "import wandb; print('✓ wandb import successful')" 2>&1
if [[ $? -ne 0 ]]; then
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --lr_scheduler_patience)
            ...
            ;;
        --lr_scheduler_factor)
            LR_SCHEDULER_FACTOR="$2"
            shift 2
            ;;
        --lr_scheduler_min_lr)
            LR_SCHEDULER_MIN_LR="$2"
            shift 2
            ;;
        --help|-h)
            ...
            echo "  --use_lr_scheduler BOOL       Enable ReduceLROnPlateau scheduler (default: false)"
            echo "  --lr_scheduler_patience N     Epochs with no improvement before LR reduction (default: 10)"
            echo "  --lr_scheduler_factor F       Factor to reduce LR (new_lr = lr * factor) (default: 0.5)"
            echo "  --lr_scheduler_min_lr LR      Minimum learning rate (default: 1e-6)"
            echo ""
...
TRAIN_ARGS="$TRAIN_ARGS --joint_features $JOINT_FEATURES"
TRAIN_ARGS="$TRAIN_ARGS --attention $ATTENTION --condition_on_fg $CONDITION_ON_FG"
TRAIN_ARGS="$TRAIN_ARGS --use_lr_scheduler $USE_LR_SCHEDULER"
TRAIN_ARGS="$TRAIN_ARGS --lr_scheduler_patience $LR_SCHEDULER_PATIENCE"
TRAIN_ARGS="$TRAIN_ARGS --lr_scheduler_factor $LR_SCHEDULER_FACTOR"
TRAIN_ARGS="$TRAIN_ARGS --lr_scheduler_min_lr $LR_SCHEDULER_MIN_LR"

$pybin train_model.py $TRAIN_ARGS
