...
def _run_autogrid(gpf_path, protein_pdbqt_dir):
    ...
    for path in possible_paths:
        ...
        if Path(path).is_absolute():
            ...
            # For executable names, check if they're in PATH
            if which(path):
                autogrid_cmd = path
                break
    
    if autogrid_cmd is None:
        print(f"Warning: autogrid4 not found. Tried: {', '.join(possible_paths)}")
        raise RuntimeError(
...
def _calculate_autodock_gpu_score(
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if autodock_gpu_path is None:
            possible_paths = [
                "/mnt/SSD3/cong_nguyen/Code/pharmacy_code/AutoDock-GPU/bin/autodock_gpu_64wi",
            ...
            if autodock_cmd is None:
                raise RuntimeError(
                    f"AutoDock-GPU binary not found. Tried: {', '.join(possible_paths)}\n"
                    "Please ensure AutoDock-GPU is installed and in PATH, or provide explicit path."
                )
        else:
